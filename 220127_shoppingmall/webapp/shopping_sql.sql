...
SELECT DISTINCT oseq FROM order_view WHERE id='one' and result='1';

UPDATE order_detail SET result='2' WHERE odseq=3;


SELECT DISTINCT oseq, result FROM order_view WHERE id='one' ORDER BY result;
SELECT DISTINCT oseq, result FROM order_view WHERE id='one' ORDER BY result, oseq DESC;
SELECT DISTINCT oseq, result FROM order_view WHERE id='one';

-- paging 용 상품 추가
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image)
	VALUES(product_seq.nextVal, '저소음 벽시계', '1', 14000, 20000, 6000, '아트라운딩 저소음 벽시계', 'c1.jpg');
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image, bestyn)
	VALUES(product_seq.nextVal, '미니 벽시계', '2', 24000, 30000, 6000, '무아스 퓨어 미니 벽시계', 'c2.jpg', 'n');
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image, bestyn)
	VALUES(product_seq.nextVal, '플랫우드 벽시계', '1', 13000, 20000, 7000, '무아스 플랫우드 벽시계', 'c3.jpg', 'n');
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image, bestyn)
	VALUES(product_seq.nextVal, '팝 미러클락', '4', 30000, 40000, 10000, '무아스 LED 팝 미러클락 탁상 시계', 'c4.jpg', 'n');
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image, bestyn)
	VALUES(product_seq.nextVal, '탁상 겸용 시계', '5', 3000, 4000, 1000, 'OWC 무소음 벽 탁상 겸용 시계', 'c5.jpg', 'n');
INSERT INTO shopproduct(pseq, name, kind, price1, price2, price3, content, image, bestyn)
	VALUES(product_seq.nextVal, '대형 벽시계', '6', 50000, 60000, 10000, '아트캄보 무소음 대형 벽시계', 'c6.jpg', 'n');
...
SELECT * FROM (SELECT * FROM 
	(SELECT rownum AS rn, p.* FROM (
		(SELECT * FROM shopproduct ORDER BY pseq DESC) p)
	) WHERE rn>=?) 
WEHRE rn<=?
	
UPDATE shopmem SET id='usetest' WHERE id='test';
