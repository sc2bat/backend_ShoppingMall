...
<article>
	...
	<!-- 회원가입폼과 같이 주소검색시 결과를 받기 위해 폼 이름을 updateForm이 아니라 joinForm 으로 합니다-->
		<input type="hidden" name="command" value="memberUpdate">
		<fieldset>
			<legend>Basic Info</legend>
			<!-- id 는 수정 대상이 아니면서, submit 할때 전송될 대상이므로, input type="text"로 하되 readonly 로 설정 수정불가 -->
			<label>User ID</label>
			<input type="text" name="id" value="${loginUser.id }" readonly> <br>
		...
		<fieldset>
			...
			<label>&nbsp;</label>
			<input type="text" name="addr1" size="25" value="${addr2 }"> <br>
			<label>Phone</label>
